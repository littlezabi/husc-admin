...
<div class="settings">
  <p>
    These settings can directly affect website behaviors, so make changes
    carefully.
  </p>
  <div class="form">
    <form on:submit|preventDefault={handleForm}>
      <div class="flex-yxz">
        {#each keys as setting, index}
          {#if setting !== "_id"}
            <div
              class={typeof values[index] === "boolean"
                ? "a03x checkbox"
                : "a03x"}
            >
              <label for={setting}>
                ...
                <small>
                  ...
                  {:else if setting === "websiteName"}
                    (it will change the website name throughout the entire
                    website)
